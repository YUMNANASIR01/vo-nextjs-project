...
interface IProduct {
  ...
  price: string;
  image: string;
  rating: number;   // aap chahe to rating property bhi hata sakte hain agar use nahi karni
  category: string;
}
...
export default function FeaturedProducts() {
  ...
  useEffect(() => {
    async function FetchFeaturedProducts() {
      const response  = await fetch("http://127.0.0.1:8000/featured_Products")
